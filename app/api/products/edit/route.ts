...
import { editProductService, ProductDraft } from "@/lib/services/products.service";

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { productId, draft } = body;

    if (!productId || !draft) {
      return NextResponse.json(
        { error: "Missing productId or product draft" },
        { status: 400 }
      );
    }
    ...
    const updatedProduct = await editProductService(productId, parsedDraft);
    ...
    console.error("EDIT PRODUCT API ROUTE ERROR:", err);
    return NextResponse.json(
      { error: err.message || "Failed to edit product" },
      { status: 500 }
    );
  }
}
